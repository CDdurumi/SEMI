...
public class BoardDAO {
	...
	// 게시판 리스트 출력
				...
				public String getPageNavi(int currentPage) throws Exception{
					...
					if(needPrev) {
						sb.append("<li class='page-item'><a class= 'page-link' href='boardMainView.board?cpage="+(startNavi-1)+"'> < </a></li>");
					}

					for(int i = startNavi; i <= endNavi; i++) {
						if(currentPage == i) {
							sb.append("<li class='page-item active'><a class= 'page-link' href='/boardMainView.board?cpage="+i+"'>" + i + " </a></li>");	// 페이지 당 10개씩 보이도록 해야하기 때문에 현재 페이지를 매개변수로 보냄으로써 페이지 네비를 클릭할 때 어디로 가야하는지 알아야한다.
						}else {
							sb.append("<li class='page-item'><a class= 'page-link' href='/boardMainView.board?cpage="+i+"'>" + i + " </a></li>");
						}
					}

					if(needNext) {
						sb.append("<li class='page-item'><a class= 'page-link' href='boardMainView.board?cpage="+(endNavi+1)+"'> > </a></li>");
					}
