	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (uri.equals("/communityMain.board")) {
				...
				List<FilesDTO> filesDao = filesDAO.selectSysName(boardOption);// 애디터추천 게시글 프로필 - sys_name get(해당게시글seq와
																				// sys_name담겨 있음).
				...
				List<FilesDTO> filesDao = filesDAO.selectSysName(boardOption);// 후기 게시글 프로필 - sys_name get(해당게시글seq와
																				// sys_name담겨 있음).
				request.setAttribute("porfileList", filesDao);
				request.setAttribute("profilePath", "/files/");
				...
				/////// 숙소 화제 게시글///////
				request.setAttribute("hotlist", hotlist);
				List<FilesDTO> hFilesDao = filesDAO.selectSysName(boardOption);// 숙소 게시글 프로필 - sys_name get(해당게시글seq와
																				// sys_name담겨 있음).
				...
				String boardOption = request.getParameter("boardOption"); // 게시판 옵션(f:자유게시판, g:여행후기, j:구인구직, r:맛집,
																			// h:숙소리뷰, e:애디터추천글)
...
//				System.out.println(contents);
				...
				if (boardOption.equals("f")) {// 자유게시판 메인페이지
					...
					List<BoardDTO> editorList = dao.selectByEditorType(boardOption, editor_type);
					request.setAttribute("editorList", editorList);// 애디터추천게시글 리스트
					List<FilesDTO> filesDao = filesDAO.selectSysName(boardOption);// 애디터추천 게시글 프로필 - sys_name
																					// get(해당게시글seq와 sys_name담겨 있음).
					request.setAttribute("porfileList", filesDao);
					request.setAttribute("profilePath", "/files/");
					...
					request.setAttribute("noticeList", noticeList);
					/////// 숙소 화제 게시글///////
					List<FilesDTO> hFilesDao = filesDAO.selectSysName(boardOption);// 애디터추천 게시글 프로필 - sys_name
																					// get(해당게시글seq와 sys_name담겨 있음).
					request.setAttribute("housePorfileList", hFilesDao);
...
//					response.sendRedirect(referer);
			...
			} else if (uri.equals("/replycnt.board")) {
				// 댓글 수
				String seq = request.getParameter("seq");
				System.out.println(seq);
				int reTotalCnt = replyDao.selectReTotalCnt(seq);
				PrintWriter pw = response.getWriter();
				pw.append(g.toJson(reTotalCnt));
			}
